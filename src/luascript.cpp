...
{
	// get: monsterType:nameDescription() set: monsterType:nameDe
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
		...
		if (moveevent->getEventType() == MOVE_EVENT_EQUIP || moveevent->getEventType() == MOVE_EVENT_DEEQUIP) {
			...
			if (!moveevent->getSlot()) {
				...
				if (tmpStr == "head") {
					...
					std::cout << "[Warning - MoveEvent::configureMoveEvent] Unknown slot type: " << moveevent->getSlot() << std::endl;
				}
			}
		}
		pushBoolean(L, true);
